...
#+begin_example
  ...
  Traceback (most recent call last):
    File "/tmp/babel-0NE78C/python-uG2oyE", line 201, in <module>
      ship_it(2)
      ~~~~~~~^^^
    File "/tmp/babel-0NE78C/python-uG2oyE", line 191, in ship_it
      session.commit()
      ~~~~~~~~~~~~~~^^
    File "/home/octopus/study/Python/sqlalchemy-study/.venv/lib/python3.14/site-packages/sqlalchemy/orm/session.py", line 2035, in commit
      trans.commit(_to_root=True)
  ...
  ERROR:root:нужен rollback
  Traceback (most recent call last):
    File "/tmp/babel-0NE78C/python-uG2oyE", line 206, in <module>
      print(session.execute(stmt).all())
            ~~~~~~~~~~~~~~~^^^^^^
    File "/home/octopus/study/Python/sqlalchemy-study/.venv/lib/python3.14/site-packages/sqlalchemy/orm/session.py", line 2373, in execute
      return self._execute_internal(
...
** SQLAlchemy v1
:PROPERTIES:
:header-args:python: :session *ch10-v1* :tangle ch10-v1.py :cache yes :noweb yes
:END:
#+BEGIN_SRC python :results table :shebang "#!/usr/bin/env python3"
  ...
  [(k,) for k in Base.classes.keys()]
#+END_SRC

#+RESULTS[32bdca5524458c540535c004a2d593ed04a87d7b]:
| Playlist    |
| Track       |
| Customer    |
| InvoiceLine |
...
#+BEGIN_SRC python :results table
  ...
  [(artist.ArtistId, artist.Name) for artist in session.query(Artist).limit(10)]
#+END_SRC

#+RESULTS[983002791b292f46a0452544998df191dec7c602]:
|  1 | AC/DC                |
|  2 | Accept               |
|  3 | Aerosmith            |
|  4 | Alanis Morissette    |
...
#+BEGIN_SRC python :results table
  ...
  [(artist.Name, album.Title)for album in artist.album_collection]
#+END_SRC

#+RESULTS[9d2f8b99a53eaa7596304d49c1c8cfe470d9a8c8]:
| AC/DC | For Those About To Rock We Salute You |
| AC/DC | Let There Be Rock                     |

** SQLAlchemy v2
:PROPERTIES:
:header-args:python: :session *ch10-v2* :tangle ch10-v2.py :cache yes :noweb yes
:END:
...
#+BEGIN_SRC python :shebang "#!/usr/bin/env python3" :results none
  ...
  # 2. Движок и рефлексия (2.0 стиль: autoload_with)
  engine = create_engine("sqlite:///Chinook_Sqlite.sqlite")
  Base.prepare(autoload_with=engine)

  # 3. Список имён классов
  class_names = list(Base.classes.keys())

  # 4. Работа с сессией через контекстный менеджер (рекомендуется)
  with Session(engine) as session:
      Artist = Base.classes.Artist
      Album = Base.classes.Album

      # 5. Выборка с limit — scalars() возвращает ORM-объекты напрямую
      artists = session.scalars(select(Artist).limit(10)).all()
      artist_data = [(a.ArtistId, a.Name) for a in artists]

      # 6. Первый артист и его альбомы (ленивая загрузка через relationship)
      artist = session.scalars(select(Artist)).first()
      if artist:
          albums_data = [(artist.Name, album.Title) for album in artist.album_collection]
#+END_SRC
